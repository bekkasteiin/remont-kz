...
class _ShowPersonTaskState extends State<ShowPersonTask> {
  ...
  Widget build(BuildContext context) {
    List<ChatList> rejectList = widget.items.where((element) => element.status == 'TASK_REJECTED').toList();
    List<ChatList> activeTask = widget.items.where((element) => element.status != 'TASK_REJECTED').toList();
    return Scaffold(
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: widget.publication.isContractual
                              ? Text("договорная")
                              : Text("${widget.publication.price.toInt().toString()} ₸"),
                        ),
                      ],
                    ),
