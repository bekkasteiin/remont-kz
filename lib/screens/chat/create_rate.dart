...
class _CreateRateWorkerState extends State<CreateRateWorker> {
  ...
  List<TypeEmoji> typeCarBodyList = [
    TypeEmoji(image: "assets/icons/emoji_best.png", name: 'Отлично', rate: 3),
    TypeEmoji(image: "assets/icons/emoji_good.png", name: 'Хорошо', rate: 2),
    TypeEmoji(image: "assets/icons/emoji_angry.png", name: 'Плохо', rate: 1),
  ];
  var carBodyType;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      floatingActionButton: FutureBuilder(
          future: widget.list?.type == "TASK"
              ?RestServices().getTaskById(widget.list?.typeId ?? 0)
          : RestServices().getPublicationById(widget.list?.typeId ?? 0),
                      ...
                      if (_formKey.currentState!.validate()) {
                        var api = await RestServices().createNewComment(
                          TaskComment(
                            ...
                            files: files,
                            executorUsername: snapshot.data.user.username,
                            categoryId: snapshot.data.categoryId,
                              chatRoomId: widget.list?.id ?? 0
                          ),
                        );
                        if (api == 200) {
